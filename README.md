...
При использования такого подхода возникает ряд сложностей и (субъективно) неявной логики, решением которых и является подход описанный ниже. Ниже перечислены проблемы который предлагается решить.
...
**Примечаение**. `RenderObject` класс из фрейморка кроссплатформенного ui.
...
Самим компонентом при этом служит обертка реализующая класс `React.Component`, которая скрыта от пользователя. Обертка называется `Box`. Кроме делегирования методов инстансу виджета, она также хранит состояние (об этом в следующем разделе) и может хранить пользовательские данные (об этом также ниже).
...
Т.к. дочерние элементы объявляются в виде лямбды, есть возможность объявить ее suspend, и, вместо списка, возвращать блокирущий итератор (или даже не блокирущий). На данный момент это не рализовано.
...
Важно помнить отличие виджетов от компонентов: сам виджет будет создан при вызове `render` дочернего компонета и может быть не добавлен в дерево элементов `React`. Для того чтобы провести инициализацию только после добавления в дерево необходимо переопределить `init`. Аналагично методу `init`, `dispose` вызывается при удалении. После вызова `dispose` виджет не может быть использован повторно.
...
abstract class Observer<T : Any>(
 ...
 /**
  * Вызывается при добавлении виджета в дерево, в случае если `BoxData` не существует 
  */
  override fun initBoxData(box: WidgetBox<T>): WidgetBoxData
      = BoxData(box, observable)

  /**
   * Вызывается при добавлении виджета в дерево, в случае если `BoxData` уже существует 
   */
...
В этом разделе речь пойдет про реализацию `Box`.
